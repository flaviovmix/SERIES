...
|---|---|---|---|
| `01` | **Para onde vai o salário** | entrada, gasto fixo, gasto variável e sobra. Por que a sobra vem antes de qualquer investimento. Tela: a barra do mês com fatias arrastáveis (aluguel, mercado, lazer), e a sobra cresce ou some conforme o arrasto | 🟠 **16/09: pesquisa, planta de 12 telas, trio e áudio v1 prontos.** Pesquisa em fonte primária (8 dos 9 itens); filmes da etapa levantados por busca com a ressalva de cada um; trio **reprovado pelo verificador na primeira volta** e corrigido em 16 pontos (a conta do INSS pulava a faixa de 12%, os 92,6% estavam sobre a base errada, 28,85% virou 28,8 e a chamada de extra abria o assunto). Áudio **"Para onde vai seu salário de verdade", 15:23**, no notebook `81cda56f`. **Mapa de telas conferido: 12 telas e 11 pedidos, batendo com a planta**, e a frase de precisão do imposto saiu inteira ("a tabela em si não mudou a isenção"). Ressalvas confirmadas no áudio com o modelo maior, pra ele ouvir: o Datafolha saiu como **"julho de 2020"** quando o campo foi em julho de 2025 (12:58); **"PTU"** em vez de IPTU (04:39); "é uma temática fria" onde seria matemática fria (04:50); e um **"genial"**, palavra proibida, na tela 8, com autocorreção na frase seguinte. Falsos alarmes do modelo pequeno, já descartados: "R$ 2.026", "39,13" (o áudio diz 39,3), "Super Individamento" e "Contrachec". 🟢 **PÁGINA NO AR em 16/09**, com as 12 telas montadas em cima do `_telas.md` e quatro widgets próprios (a régua do bruto ao líquido, a barra do mês, os doze meses do ano que não fecha e a prática da sobra na frente). ⚠️ Subiu **sem ilustração nenhuma**, só com a capa emprestada da série: os dados viraram desenho da própria página (a tabela do INSS, as fatias da POF, as duas pontas) e eu tratei os widgets como se fossem a imagem de toda tela. Ele reparou na hora ("a aula ficou sem imagem"), e a planta deste mesmo episódio já mandava o contrário. 🟢 **Corrigido no mesmo dia (`ab74826`):** as **8 ilustrações** das telas de conteúdo, geradas no Flow com os prompts guardados em `ilustracoes.md`, e o card do episódio ganhou a arte da tela final, com lupa. As telas 3, 5, 8 e 11 seguem sem foto de propósito: lá o widget interativo é a imagem, como a planta decidiu. ⚠️ Ainda faltaram três voltas, todas achadas por print dele: a arte foi parar no card do **episódio** quando o que estava vazio era o card da **etapa** (`e0cd4a1`); a ilustração em **faixa larga por cima** do conteúdo foi reprovada ("a imagem do lado ficou perfeita, a que ficou em cima ficou péssima") e todas as telas passaram a usar a imagem ao lado do texto (`ef342f5`); e a **barra da régua do INSS nunca encheu**, no telefone e no desktop, porque o `<span>` do preenchimento é inline e ignora `width` (`c0ff058`). Commits `1f8974e`, `caf3f3f`, `ab74826`, `e0cd4a1`, `ef342f5` e `c0ff058`. **Falta:** ele ouvir comparando com as telas, e decidir a P17 (arte das etapas) |
