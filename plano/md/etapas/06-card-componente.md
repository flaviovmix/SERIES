...
| 6.2 | O molde e o gerador, provados no `hardware.html` | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.3 | Hubs em pé: `computador.html` e `javaweb.html` | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.4 | A home deitada | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.5 | Páginas de etapa | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.6 | Extras | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.7 | `hardware-01.html` | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.8 | A D4 vira hábito | ✅ 15/09/2026 |
| 6.9 | O estado das séries, sem painel ainda | ✅ 15/09/2026, validada pelo dono no navegador em 16/09/2026 |
| 6.10 | O painel local das séries | ✅ 15/09/2026, usado pelo dono no navegador em 16/09/2026 |
...
9. **Na home a foto amplia ou leva pra série?** Hoje clicar na foto da home não faz nada, e quem clica numa vitrine costuma esperar ir pra série. **Fechada em 15/09/2026: leva pra série.** A foto da home vira link pro mesmo destino do botão (série sem `href` continua sem link), sem lupa e sem legenda; o crédito das fotos da home fica devendo pela P10 onde não houver. **Mudada pelo dono no mesmo dia, depois da etapa fechada** ("adicionar a lupa para poder abrir a capa do card", na sessão da capa da Finanças): a foto da home amplia com lupa e legenda (`Série · nome · crédito`), igual a qualquer card, e quem leva pra série é só o botão. A home passou a carregar o lightbox, o gerador perdeu o modo "link pra série" e as capas da home sem crédito entraram na lista da P10 (4 capas, sem lupa até o dono dizer a origem).
...
11. **O painel é local e não publica.** Roda só na máquina do dono (`node painel/servidor.js`, escutando só `127.0.0.1`), sem dependência de npm. Ele troca o estado e roda o `gera-cards.js`; no fim mostra os arquivos que mudaram e o comando do `sobe-arquivos.sh` pronto pra copiar. Subir continua por ordem do dono (Etapa 4). Painel online, com login, é outro projeto e fica fora: o `plano.md` diz "sem backend, sem banco, sem login".
    ...
    **Virada em 16/09/2026, dita pelo dono:** *"eu quero que o painel esteja no servidor, mas ativar ou desativar vai ser apenas tornar o painel visível ou não"*. O painel passa a rodar **no servidor**, num container ao lado do `serie`, e ligar ou desligar vale na hora, sem deploy. O que derrubou a decisão antiga: tudo o que o painel faz é mostrar ou esconder uma série, e o caminho de antes (mudar aqui, subir arquivo depois) só existia porque ele morava na máquina de casa. O que **não** mudou: continua sem banco e sem cadastro de usuário. A senha é do Caddy na frente (`basic_auth`), não um login escrito por nós — o `plano.md` passou a dizer "sem backend e sem banco; a única porta com chave é o painel das séries, protegido por senha do servidor". O código é o mesmo dos dois lados: três variáveis de ambiente (`PAINEL_ESCUTA`, `PAINEL_PORTA`, `PAINEL_ENDERECO`) dizem onde ele está, e a tarja de cima da página muda junto ("Só nesta máquina · não publica nada" ou "No ar · a mudança vale na hora"), pra a tela nunca mentir sobre o que um clique faz. Montagem e cuidados: `deploy/LEIA-ME.md`.
...
| 6.4 | `index.html` | 6 (7 com a Finanças, 15/09) | 0 na fechada; 3 desde a mudança da decisão 9 (Java Web, Hardware e Finanças, capas do Flow), 4 sem crédito (P10) |
...
- **✅ Fechada em 15/09/2026, validada pelo dono no navegador em 16/09/2026.** 26 cards no dado (12 do `computador.html`, com o card Extras, e 14 do `javaweb.html`). Lupa em **6 fotos, não 8**: as capas das etapas 01 e 06 do `computador.html` (`abaco-episodio.webp`, `etapa-06.webp`) não têm crédito conhecido e ficaram sem lupa (P10). Os botões diretos das etapas 01 e 11, o "1 de 4 episódios" e todas as contagens saíram derivados da MENU, iguais aos escritos à mão. **Deltas declarados:** os dois hubs passaram a carregar `lightbox.css` e `lightbox.js` (o `--confere` cobrou; o texto desta subetapa não previa); e saíram os comentários HTML de dentro da grade ("etapa de um episódio só: o card leva direto"), porque a regra agora mora no `arvore.js` (decisão 5). **Provas:** `--confere` verde; `qa-cards.js` verde nos 3 hubs, 5 tamanhos × 2 temas, lightbox com foco e teclado em 11 artes; geometria igual à gravada. O `qa-producao-site.js` só roda contra o ar, então fica pro deploy.

### 6.4 A home deitada

- Decisão 9 fechada: a foto vira link pra série, sem lupa (mudada pelo dono ainda em 15/09, depois da etapa: ver a decisão 9). O ícone do aro segue no sprite da home (é da home, não do card).
...
- **✅ Fechada em 15/09/2026, usada pelo dono no navegador em 16/09/2026.** O `painel/` nasceu na raiz do SEIRES com `servidor.js`, `painel.html`, `painel.css`, `painel.js` e `LEIA-ME.md`, como no desenho acima. Abre com `node painel/servidor.js`, em `http://127.0.0.1:8790/`. **Provas:** `qa-painel-series.js` (novo) verde. Conferido nele:
